...
export default function EditRolePage() {
  ...
  const { data: role } = useQuery({
    ...
    queryFn: () => api.get<any>(`/roles/${params!.id}`),
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => api.patch(`/roles/${params!.id}`, data),
  ...
  useEffect(() => {
    if (role) {
      ...
      setSelectedPerms(role.rolePermissions?.map((rp: any) => rp.permissionId) || []);
    }
  }, [role]);
  ...
  if (!role) return <div className="h-32 animate-pulse rounded-lg bg-muted" />;

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Edit Role</h1>
        <p className="text-sm text-muted-foreground">Update role permissions</p>
        {role.isSystem && (
          <p className="mt-1 text-xs text-amber-600">System roles are predefined but permissions can be adjusted</p>
        )}
      </div>
